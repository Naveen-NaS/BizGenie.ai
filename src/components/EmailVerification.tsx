...
const VerifyEmail = ({ username, email, password }: VerifyEmailProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setGlobalMessage("");
    setGlobalSuccess("none");

    const code = inputsRef.current.map((input) => input?.value).join('');
    
    if (code.length !== 6) {
      setGlobalMessage('Please enter the complete 6-digit code.');
      setGlobalSuccess("false");
          ...
          <div className="max-w-md mx-auto bg-slate-800 text-center px-4 sm:px-8 py-10 rounded-xl shadow">
            ...
            <div className="text-l text-slate-200 mt-4">
              ...
              <button
                ...
                className={`font-bold text-transparent bg-clip-text ${isResendEnabled ? 'bg-gradient-to-r from-blue-500 to-violet-500' : 'text-gray-400'}`}
              >
